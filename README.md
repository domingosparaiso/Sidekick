...
Access to the web interface is protected by a login page (userName/password from 'config.h', changeable later in the setup page), backed by a session cookie; use the logout option in the menu to invalidate it. Both the login page and the file manager page can be replaced with your own by uploading a 'login.html' or a 'fs.html' file to the storage (via the file manager) with the same name; when the file isn't present, a minimal built-in page is used instead.

On ESP32 boards, the Power/HDD LED indicators on the web page update live over a WebSocket as soon as the pin state changes, instead of being polled on a timer. If the board has `HOST_SERIAL` configured (see 'config.h'), a remote serial console is also available on the web page, bridged to the browser over its own WebSocket, useful to interact with a host device attached to that UART. Both features need a WebSocket-capable webserver, so they are only available on ESP32; on ESP8266 the LED status is still readable, it's just not pushed automatically, and the serial console isn't available. See 'Doc/Webserver API.md' for the full endpoint list.
